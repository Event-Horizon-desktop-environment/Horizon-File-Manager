...
public:
  ...
  using KeyboardKeyCb = std::function<void(const KeyboardEvent&)>;
  ...
  void set_keyboard_key_cb(KeyboardKeyCb cb) { keyCb_ = std::move(cb); }
  ...
  [[nodiscard]] xkb_state* xkb_state_ptr() const { return xkbState_; }
...
private:
  ...
  static void kb_key(void* data, wl_keyboard*, uint32_t, uint32_t, uint32_t keycode, uint32_t state);
  static void kb_modifiers(void* data, wl_keyboard*, uint32_t, uint32_t depressed, uint32_t latched, uint32_t locked,
                           uint32_t group);
  static void kb_repeat_info(void*, wl_keyboard*, int32_t, int32_t) {}
  static constexpr wl_keyboard_listener kKeyboardListener_ = {
      .keymap = kb_keymap,
      .enter = kb_enter,
      .leave = kb_leave,
      .key = kb_key,
