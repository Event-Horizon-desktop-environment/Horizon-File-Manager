...
static int run_compress_piped(const std::string& cmd,
  ...
  auto publish = [&]() {
    ...
    if (in_frac > 0.99) in_frac = 0.99; // 1.0 is reserved for verified completion
...
void execute_compress_async(AppState& app) {
  ...
  std::thread([&app, prog, cmd, archive, sources, format_idx = app.compress_format,
               level = app.compress_level, threads = app.compress_threads]() {
    // Phase 1: enumerate once. The in-process engine and the external-tool
    // fallback share this plan (rel names double as stdout match keys).
    std::string parent = common_parent_dir(sources);
    EnginePlan plan = engine_plan_sources(sources, parent);
    std::unordered_map<std::string, uint64_t> sizes;
    sizes.reserve(plan.entries.size() * 2 + 1);
    for (auto& e : plan.entries)
      if (e.kind == 'f') sizes.emplace(e.rel, e.size);
    int total = plan.file_count;
    uint64_t total_bytes = plan.total_bytes;
    ...
    if (engine_ok) {
      ...
      ret = 1; // engine attempted and failed: report, don't double-run
    }
